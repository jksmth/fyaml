...
verify-examples: build ## verify example directories
	...
	@./fyaml examples/with-shared-includes/config1 --chroot examples/with-shared-includes --enable-includes > /dev/null && echo "✓ examples/with-shared-includes" || (echo "✗ examples/with-shared-includes failed" && exit 1)
